POLYGON = {
    ...
    "coordinates" : [
        [
            [-100, 20], [-100, 60], [-80, 40], [-60, 60], [-40, 40], [-40, 20], [-100, 20]
        ],
        [
            [50, 10], [50, 20], [80, 20], [80, 10], [50, 10]
        ]
    ]
}
...
LINEAR_SPACE ={
    ...
    check_cord_in_polygon : function(polygon, x, y){
        ...
        for(poly=0; poly < N; poly++){
            v1 = polygon[poly + 1]
            v2 = polygon[poly]
            if((v2[1] > y) != (v1[1] > y) && x < (v1[0] - v2[0]) * (y - v2[1]) / (v1[1] - v2[1]) + v2[0]){
                flag = !flag
            }
        }
        return flag
    ...
    get_nearest_polygon : function(line, polygons){
        ...
        polygons.forEach(polygon => {
            ...
            for (p = 0; p < polygon.length - 1; p++){
                intersect = LINEAR_SPACE.get_intersect_cords(line[0][0], line[0][1], line[1][0], line[1][1], polygon[p][0], polygon[p][1], polygon[p + 1][0],  polygon[p + 1][1])
                if(intersect[0]){
                    f = 1
                }
            }
            ...
            if(f == 0){
                ...
                for (let poly_cord = 0; poly_cord < polygon.length - 1; poly_cord++) {
                    ...
                    dot_dist.push(this.get_pDistance(element1[0], element1[1], line[0][0], line[0][1], line[1][0], line[1][1]))
                }
            }
...
function run(lines, polygons)
{
    ...
    for(line = 0; line < lines.length -1  ; line++){
        ...
        for(j =0; j < polygons.length ; j++){
            ...
            if(LINEAR_SPACE.check_cord_in_polygon(polygon, line_cords[0][0], line_cords[0][1])){
            ...
            for (p_cord = 0; p_cord < polygon.length - 1; p_cord++){
                intersect = LINEAR_SPACE.get_intersect_cords(line_cords[0][0], line_cords[0][1], line_cords[1][0], line_cords[1][1], polygon[p_cord][0], polygon[p_cord][1], polygon[p_cord + 1][0],  polygon[p_cord + 1][1])
                if(intersect[0]){
            ...
            if(LINEAR_SPACE.check_cord_in_polygon(polygon, line_cords[1][0], line_cords[1][1])){
        ...
        display.push(["line" +  line,"Coordimates of line that is in the polygon or intersects it ==> " + cords])
        ...
        for (let cord = 0; cord < cords.length - 1; cord++) {
            ...
            if(cord1 != 'nn' && cord1 != '|'){
                ...
                if(properties.length != 0 && properties[properties.length-1][0] == "line" + line){
                    properties[properties.length-1][1] += LINEAR_SPACE.get_cords_distance(cord1[0], cord1[1], cord2[0], cord2[1])
                }
                else{
                    properties.push(["line" + line,LINEAR_SPACE.get_cords_distance(cord1[0], cord1[1], cord2[0], cord2[1])])
                }
                cord ++
            }
            else{
                //if properties length equals zero or line be separate from polygons add zero to our array
                if(properties.length == 0 || properties.length != 0 && properties[properties.length-1][0] != "line" + line){
                    properties.push(["line" + line, 0])
                }
            }
        }
        display[line].push("Line's Area in Polygons == >  " + properties[line][1] * POLYLINE_WIDTH )
        display[line].push("Line Coordinates ==>  " + lines[line] + "  "+ lines[line+1])
        display[line].push("line distances from polygons ==> " + LINEAR_SPACE.get_nearest_polygon([lines[line], lines[line+1]], polygons))
        properties[line].push(properties[line][1] * POLYLINE_WIDTH )
        properties[line].push([lines[line] , lines[line+1]])
        properties[line].push(LINEAR_SPACE.get_nearest_polygon([lines[line], lines[line+1]], polygons))
    }
...
function senario2(answers){
    for (let line = 0; line < answers[1].length; line++) {
        ...
        if(element[2] == 0){
            ...
            if(element[2] / LINEAR_SPACE.get_cords_distance(element[3][0][0], element[3][0][1], element[3][1][0], element[3][1][1]) < 0.05  ){
                points += 8
            }
            if(element[2] / LINEAR_SPACE.get_cords_distance(element[3][0][0], element[3][0][1], element[3][1][0], element[3][1][1]) < 0.1){
                points+=2
            }
        }
