var pr = require('./updated_project');
...
Genetic = {
    ...
    Selection2 : {
        "RouletteWheel" :function(pop, totalFitness){ 
            return [Genetic.Selection1.RouletteWheel(pop, totalFitness), Genetic.Selection1.RouletteWheel(pop, totalFitness)]
        },
        "Tournament2": function(pop) {
            return [Genetic.Selection1.Tournament2(pop), Genetic.Selection1.Tournament2(pop)];
        },
        "Tournament3": function(pop) {
            return [Genetic.Selection1.Tournament3(pop), Genetic.Selection1.Tournament3( pop)];
        }, 
        "Random": function (pop) {
    ...
    Crossover : {
        ...
        "TwoPoint" : function(mother, father){
            ...
            son = son.concat(father.slice(0, ca), mother.slice(ca, cb), father.slice(cb))  
            daughter = daughter.concat(mother.slice(0, ca), father.slice(ca, cb), mother.slice(cb))
        ...
        "PartiallyMapped" : function (father, mother) {
            ...
            let offspring = [Array.from(father), Array.from(mother)];
            ...
            for (let i = cb; i < father.length; i++) {
              while (offspring[0][i] in map1) {
                offspring[0][i] = map1[offspring[0][i]];
              }
              while (offspring[1][i] in map2) {
...
Problem = {
    ...
    Fitness : function(entity){
        ...
        info = pr.run(entity, pr.POLYGON.coordinates)
        var fitness = 0;
        //get lenght reduction senarios point
        fitness += configuration.lengthReduction(configuration.sourceLen ,info.totalLength)
        polygon1 = []
        polygon2 = []
        polygon3 = []
        var neededArea = 0
        ...
        function findMin(arr, num){
            let min = arr[0][num]
            for (let g = 1, len=arr.length; g < len; g++) {
                let v = arr[g][num];
                min = (v < min) ? v : min;
            }
            return min
        ...
        function findSum (arr, num){
            let sum = arr[0][num]
            for(let d = 1, len=arr.length; d < len; d++){
                sum += arr[d][num]
            }
            return sum
        }
        ...
        for (let i = 0; i < pr.POLYGON.type.length; i++) {
            if(POLYGON.type[i] == "Polygon1") polygon1.push(this.PolyPoint.Polygon1(findMin(info.distances , i)))
            if(POLYGON.type[i] == "Polygon2"){
                polygon2.push(this.PolyPoint.Polygon2(findMin(info.distances , i), info.polygonAreas[i])[0])
                neededArea += this.PolyPoint.Polygon2(findMin(info.distances , i), info.polygonAreas[i])[1] 
            }
            if(POLYGON.type[i] == "Polygon3") polygon3.push(this.PolyPoint.Polygon3(info.totalLength, findSum(info.lengthInPolygons, i) ,findMin(info.distances , i)))
        }
    ...
    Seed : function (coordinates, count){
            ...
            while (individual.length < count) {
              rangeY = [Math.min(coordinates[0][1], coordinates[1][1]), Math.max(coordinates[0][1], coordinates[1][1])]
              rangeX = [Math.min(coordinates[0][0], coordinates[1][0]), Math.max(coordinates[0][0], coordinates[1][0])]
              rand = [Math.floor(Math.random() * (rangeX[1] - rangeX[0] + 1) + rangeX[0]), Math.floor(Math.random() * (rangeY[1] - rangeY[0] + 1) + rangeY[0])]
...
function start(){
	...
	for (i=0; i < this.configuration.size; ++i)  {
        entities.push(Clone(Problem.Seed(configuration.coordinates, configuration.individualCount)));
    }
    ...
    for (let i = 0; i < configuration.iterations; i++) {
        function calFitness(population){
            ...
            for (let i = 0; i < population.length - 1; i++) {
                ...
                calculated.push({"fitness": calculation[0], "entity": population[i], "info" : calculation[1] })
            }
        ...
        function mutates(entity) {
            ...
            return Math.random() <= configuration.mutationPr ? configuration.mutation(Clone(entity)) : entity;
        }
		var newPop = [];
		...
		while (newPop.length < configuration.size) {
			...
			if (
                ...
                var parents =  (configuration.select2.name == "RouletteWheel") ? configuration.select2(pop, totalFit) : configuration.select2(pop)
				var children = configuration.crossover(Clone(parents[0].entity), Clone(parents[1].entity)).map(mutates);
                newPop.push(children[0], children[1]);

            }
            //mutation
            else {
                var parent =  (configuration.select1.name == "RouletteWheel") ? configuration.select1(pop, totalFit) : configuration.select1(pop)
